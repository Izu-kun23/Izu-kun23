...
Run it once; it is not on a schedule, unlike scripts/generate_stats.py.
    ...
    .venv/bin/python scripts/make_portrait.py source.png --crop 540,0,1080,600
    .venv/bin/python scripts/embed_portrait_font.py      # inline the font, see below
...
def prep(path, crop=None):
    ...
    # A pre-cut RGBA source already has a matte. rembg would only smear it,
    # and it saves the 176 MB model download on regenerate.
    alpha_src = np.array(src.split()[-1])
    if (alpha_src < 20).mean() > 0.08:
        cut = src
    else:
